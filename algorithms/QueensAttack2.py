...
# obstacles in the diaognal to the queen
# The last element in a tuple indicates whether there is any obstacle in the diagonal direction
diaognal_upper_left = (n,1, False)
diaognal_bottom_right = (1,n, False)
diaognal_upper_right = (n,n, False)
diaognal_bottom_left = (1,1, False)
...
if k != 0:
            ...
            if abs(rQueen - rObstacle) == abs(cQueen - cObstacle):
                if rObstacle - rQueen > 0:
                    ...
                    if cObstacle < cQueen:
                        ...
                        if cObstacle < diaognal_bottom_right[1]:
                            diaognal_bottom_right = (rObstacle, cObstacle, True)
    ...
    # Count the attackable squares in the diaognal directions
    ...
    if diaognal_upper_left[2]:
        upper_left -= min(n - diaognal_upper_left[0] + 1, diaognal_upper_left[1])

    if diaognal_bottom_right[2]:
        bottom_right -= min(diaognal_bottom_right[0], n - diaognal_bottom_right[1] + 1)

    if diaognal_upper_right[2]:
        upper_right -= min(n - diaognal_upper_right[0] + 1, n - diaognal_upper_right[1] + 1)

    if diaognal_bottom_left[2]:
        bottom_left -= min(diaognal_bottom_left[0], diaognal_bottom_left[1])
